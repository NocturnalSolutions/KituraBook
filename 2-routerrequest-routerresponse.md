...
Note that we’re using the `all()` method here instead of the `get()` one as we’ve used before. Using `get()` tells Kitura we want our handler to fire only on GET requests, whereas using `all()` tells Kitura we want it to fire on *all* request methods - GET, POST, and so on.
